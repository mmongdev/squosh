...
import { compressFilesBatch, BatchProgress } from './batch-compress';
import {
  SourceImage,
  decodeImage,
  preprocessImage,
...
import { filesToZipBlob } from 'shared/util/zip-files';
import { downloadBlob } from 'shared/util/download-blob';
...
export default class Compress extends Component<Props, State> {
  ...
  private getBatchSideIndex(): 0 | 1 {
    const [left, right] = this.state.sides;
    if (right.latestSettings.encoderState) return 1;
    if (left.latestSettings.encoderState) return 0;
    return 1;
  };

  private onDownloadAll = async () => {
    const { preprocessorState, sides, batchDownloading } = this.state;
    const { files } = this.props;
    if (files.length <= 1 || batchDownloading) return;

    const sideIndex = this.getBatchSideIndex();
    const { processorState, encoderState } = sides[sideIndex].latestSettings;

    this.batchAbortController?.abort();
    this.batchAbortController = new AbortController();
    const signal = this.batchAbortController.signal;

    this.setState({ batchDownloading: true, batchProgress: undefined });
    ...
    try {
      const compressed = await compressFilesBatch(
        ...
        processorState,
        encoderState,
        this.workerBridges[sideIndex],
        (progress) => this.setState({ batchProgress: progress }),
      );

      const zipBlob = await filesToZipBlob(compressed);
      const label = encoderState
        ? encoderMap[encoderState.type].meta.label
        : 'original';
      downloadBlob(
        zipBlob,
        `squoosh-${label.replace(/\s+/g, '-').toLowerCase()}.zip`,
      );

      ga('send', 'event', 'compression', 'batch-download', {
        metric1: compressed.length,
      });

      this.props.showSnack(`Downloaded ${compressed.length} images as ZIP`);
    } catch (err) {
      if (err instanceof Error && err.name === 'AbortError') return;
      this.props.showSnack(`Batch download failed: ${err}`);
    } finally {
      this.setState({ batchDownloading: false, batchProgress: undefined });
    }
  };
